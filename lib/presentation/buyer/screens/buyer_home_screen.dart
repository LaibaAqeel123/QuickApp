...
class _BuyerHomeScreenState extends State<BuyerHomeScreen> {
  ...
  @override
  void initState() { super.initState(); _loadProfile(); _loadCategories(); _loadFeatured(); }
  ...
  Future<void> _loadProfile() async {
    ...
    setState(() {
      _businessName = business.isNotEmpty ? business
          : '$first $last'.trim().isNotEmpty ? '$first $last'.trim() : 'My Account';
      _profileLoaded = true;
    });
  }
  ...
  static const List<Map<String, dynamic>> _fallback = [
    {'id': null, 'name': 'Bakery'},     {'id': null, 'name': 'Meat'},
    {'id': null, 'name': 'Dairy'},      {'id': null, 'name': 'Fruit & Veg'},
    {'id': null, 'name': 'Frozen'},     {'id': null, 'name': 'Dry Items'},
    {'id': null, 'name': 'Beverages'},  {'id': null, 'name': 'More'},
  ];
  ...
  Future<void> _loadFeatured() async {
    ...
    if (r.success && r.data != null) {
      ...
      setState(() { _featured = raw.whereType<Map<String, dynamic>>().take(4).toList(); _featuredLoading = false; });
    } else { setState(() => _featuredLoading = false); }
  }

  void _openCategory(Map<String, dynamic> cat) {
    final name  = cat['name']?.toString() ?? '';
    final id    = cat['id'];
    final intId = id is int ? id : (id != null ? int.tryParse(id.toString()) : null);
    Navigator.push(context, MaterialPageRoute(
      builder: (_) => CategoryProductsScreen(category: name, categoryId: intId)));
  }
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: RefreshIndicator(
          onRefresh: () async => await Future.wait([_loadProfile(), _loadCategories(), _loadFeatured()]),
          child: SingleChildScrollView(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      const Text('Welcome back 👋', style: TextStyle(fontSize: 14, color: AppColors.white)),
                      const SizedBox(height: 4),
                      Text(_profileLoaded && _businessName.isNotEmpty ? _businessName : 'My Restaurant',
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Row(children: [
                        ...
                        const Expanded(child: Text('Search products, suppliers...', style: TextStyle(color: AppColors.textHint, fontSize: 15))),
              ...
              SizedBox(height: 44, child: ListView(
                ...
                children: ['Bakery', 'Chicken', 'Dairy', 'Fresh Veg', 'Rice'].map((q) =>
                  Padding(padding: const EdgeInsets.only(right: 8), child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Text(q, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.primary)),
                    ),
                  ))
                ).toList(),
              )),
              ...
              Padding(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  ...
                  if (_categoriesLoading)
                    ...
                    TextButton(onPressed: () => widget.onBrowseTap(), child: const Text('View All')),
                ]),
              ),
              ...
              Padding(
                ...
                child: _categoriesLoading
                    ...
                    : GridView.builder(
                        shrinkWrap: true, physics: const NeverScrollableScrollPhysics(),
                        // childAspectRatio: 0.85 gives enough height for image + label
                        gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                            crossAxisCount: 4, mainAxisSpacing: 12, crossAxisSpacing: 12, childAspectRatio: 0.85),
                        // No limit — all categories from API
                        itemCount: _categories.length,
                        itemBuilder: (_, i) {
                          final cat = _categories[i];
                          return _CategoryCard(
                            label:    cat['name']?.toString() ?? '',
                            imageUrl: cat['imageUrl']?.toString() ?? cat['image']?.toString(),
              ...
              Padding(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  const Text('Nearby Suppliers', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
                  TextButton(onPressed: () => widget.onBrowseTap(), child: const Text('View All')),
                ]),
              ),
              ...
              SizedBox(height: 210, child: ListView.builder(
                ...
                itemBuilder: (_, i) => _SupplierCard(
                  name: 'Premium Wholesale ${i + 1}', distance: '${(i + 1) * 0.5} km',
                  rating: 4.5 + (i * 0.1), products: '250+',
                  onTap: () => Navigator.push(context, MaterialPageRoute(
                    builder: (_) => SupplierDetailScreen(supplierName: 'Premium Wholesale ${i + 1}'))),
                ),
              )),
                  ...
                  child: GridView.builder(
                    ...
                    itemBuilder: (_, i) {
                      if (_featured.isNotEmpty) {
                        final p = _featured[i];
                        return _ProductCard(
                          name: _pName(p), price: _pPrice(p), supplier: _pSupplier(p), category: _pCategory(p),
                      ...
                      final f = fb[i];
                      return _ProductCard(name: f['name'] as String, price: (f['price'] as num).toDouble(),
                          supplier: f['supplier'] as String, category: f['category'] as String);
                    },
                  ),
                ),
...
// ══════════════════════════════════════════════════════════
//  CATEGORY CARD — shows real API image or falls back to icon
// ══════════════════════════════════════════════════════════
              ...
              ? ClipRRect(
                  ...
                  child: Image.network(
                    ...
                    loadingBuilder: (_, child, prog) =>
                        prog == null ? child : Center(child: Icon(_icon(), size: 26, color: AppColors.primary)),
                    errorBuilder: (_, __, ___) => Center(child: Icon(_icon(), size: 26, color: AppColors.primary)),
                  ))
...
class _SupplierCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(height: 95, decoration: const BoxDecoration(color: AppColors.surfaceLight,
            borderRadius: BorderRadius.vertical(top: Radius.circular(16))),
            child: const Center(child: Icon(Icons.store, size: 48, color: AppColors.primary))),
        Padding(padding: const EdgeInsets.all(10), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            const Icon(Icons.star, size: 13, color: AppColors.warning), const SizedBox(width: 3),
            ...
            const Icon(Icons.location_on, size: 13, color: AppColors.textHint), const SizedBox(width: 3),
          ...
          Container(
            ...
            child: Text('$products Products', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: AppColors.primary)),
          ),
        ])),
      ]),
    ),
...
class _ProductCard extends StatelessWidget {
  ...
  const _ProductCard({required this.name, required this.price,
      required this.supplier, required this.category, this.imageUrl, this.onTap});
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(16),
          border: Border.all(color: AppColors.border)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(height: 110, decoration: const BoxDecoration(color: AppColors.surfaceLight,
            borderRadius: BorderRadius.vertical(top: Radius.circular(16))),
          alignment: Alignment.center,
          child: imageUrl != null && imageUrl!.isNotEmpty
              ? ClipRRect(borderRadius: const BorderRadius.vertical(top: Radius.circular(16)),
                  child: Image.network(imageUrl!, fit: BoxFit.cover, width: double.infinity, height: double.infinity,
                    errorBuilder: (_, __, ___) => Icon(_icon(), size: 55, color: AppColors.primary)))
        ...
        Expanded(child: Padding(padding: const EdgeInsets.all(10), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 4),
          if (supplier.isNotEmpty) Text(supplier, style: const TextStyle(fontSize: 11, color: AppColors.textSecondary),
              maxLines: 1, overflow: TextOverflow.ellipsis),
          const Spacer(),
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Container(padding: const EdgeInsets.all(6),
              decoration: BoxDecoration(color: onTap != null ? AppColors.primary : AppColors.border, borderRadius: BorderRadius.circular(8)),
