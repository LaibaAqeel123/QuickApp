...
class _OrderTrackingScreenState extends State<OrderTrackingScreen> {
  ...
  // FROM CURRENT (HEAD) — better: correctly computes driver en-route state
  bool _computeDriverEnRoute() {
    final s = _statusLabel.toLowerCase();
    return s == 'out for delivery' || s == '2';
  }

  // FROM CURRENT (HEAD) — better: checks delivery API status for picked-up state
    ...
    if (r.success && r.data != null) {
      ...
      if (deliveryResult.success && deliveryResult.data != null) {
        ...
        if (driverPickedUp && dId.isNotEmpty) {
          ...
          _startPolling();
          // Then try SignalR for real-time updates
  ...
  // ── FROM CURRENT: Read driver lat/lng directly from delivery API response ──
  ...
  // ══════════════════════════════════════════════════════
  //  SIGNALR — FROM CURRENT (much more robust than incoming)
  //  Tries WebSockets+skipNeg → WebSockets → LongPolling
  //  Registers all known event name variants
  // ══════════════════════════════════════════════════════
  ...
  Future<void> _resubscribe(HubConnection hub, String delivId) async {
    final orderId = _orderId; // also try subscribing by orderId
    ...
    // Also try with orderId — hub might broadcast to orderId group
  ...
  Future<void> _connectSignalR() async {
    ...
    // Try transport methods in order: best → most compatible
    await _tryConnect(
                    ...
                    if (_isDriverEnRoute && _etaMinutes != null)
                      Container(
                        ...
                        child: Row(children: [
                          ...
                          Text(
                            'Driver arriving in '
                            '${_etaMinutes!.toStringAsFixed(0)} minutes',
                            style: const TextStyle(
                                fontSize: 14,
  ...
  // ══════════════════════════════════════════════════════
  //  MAP CONTENT — FROM CURRENT (cleaner state handling)
  // ══════════════════════════════════════════════════════
  Widget _buildMapContent() {
    if (!_isDriverEnRoute) {
      return _MapPlaceholder(statusLabel: _statusLabel);
    }
        ...
        child: const Center(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 12),
              Text('Connecting to driver location...',
                  style: TextStyle(
    ...
    return Stack(
      children: [
        ...
        // SignalR live/connecting badge — FROM INCOMING (nice UX addition)
        Positioned(
          top: 10, right: 10,
          child: Container(
            padding:
