...
class _BuyerProfileScreenState extends State<BuyerProfileScreen> {
  ...
  Future<void> _loadAddressCount() async {
    ...
    if (!mounted) return;
    if (result.success) {
      setState(() => _addressCount = result.data?.length ?? 0);
    }
  }
          ...
          _SectionCard(label: 'Account', children: [
            ...
            _MenuItem(
              ...
              subtitle: _addressCount > 0
                  ? '$_addressCount saved address${_addressCount == 1 ? '' : 'es'}'
                  : 'Manage delivery addresses',
              trailingWidget: _addressCount > 0
                  ? Container(
                      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                      decoration: BoxDecoration(
                          color: AppColors.primary,
                          borderRadius: BorderRadius.circular(10)),
                      child: Text('$_addressCount',
...
class _MenuItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ListTile(
    ...
    trailing: trailingWidget ??
        const Icon(Icons.arrow_forward_ios, size: 15, color: AppColors.textHint),
    onTap: onTap,
  );
}