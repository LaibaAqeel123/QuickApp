...
class _PaymentScreenState extends State<PaymentScreen>
  ...
  Future<void> _payWithSavedCard() async {
    ...
    if (oid.isEmpty) {
      ...
      // Change 1 — set _groupOrderId from checkout response
      final data    = orderResult.data!;
      final orders  = data['orders'];
      final groupId = data['groupOrderId']?.toString() ?? '';

      if (orders is List && orders.isNotEmpty) {
        oid = orders.first['orderId']?.toString() ?? '';
      }
      if (oid.isEmpty) oid = data['orderId']?.toString() ?? '';
      if (groupId.isNotEmpty) setState(() => _groupOrderId = groupId);

      debugPrint('🛒 [PayWithSavedCard] Order placed! orderId: $oid groupOrderId: $groupId');
      if (oid.isEmpty) {
        setState(() => _isProcessing = false);
    ...
    final result = await AuthService.instance.payWithSavedCard(
        orderId:      (_groupOrderId == null || _groupOrderId!.isEmpty) ? oid : null,
        groupOrderId: (_groupOrderId != null && _groupOrderId!.isNotEmpty)
            ? _groupOrderId
            : null,
  ...
  Future<void> _payWithNewCard() async {
    ...
    if (oid.isEmpty) {
      ...
      // Change 2 — set _groupOrderId from checkout response
      final data    = orderResult.data!;
      final orders  = data['orders'];
      final groupId = data['groupOrderId']?.toString() ?? '';
      ...
      debugPrint('🛒 [PayWithNewCard] Order placed! orderId: $oid groupOrderId: $groupId');
      if (oid.isEmpty) {
        setState(() => _isPayingNew = false);
        _snack('Order placed but ID missing. Contact support.',
            isError: true);
  ...
  Future<void> _saveCardAfterPayment(String clientSecret) async {
    ...
    try {
      ...
      debugPrint('💳 [SaveCard] PI status   : ${paymentIntent.status}');
      debugPrint(
          '💳 [SaveCard] paymentMethodId from PI: '
              '${paymentIntent.paymentMethodId}');
      paymentMethodId = paymentIntent.paymentMethodId;
    } catch (e) {
      debugPrint('⚠️  [SaveCard] Could not retrieve PI from Stripe SDK: $e');
    }
  ...
  Future<void> _navigateToSuccess(String oid) async {
    ...
    try {
      // Change 3 — use _groupOrderId state variable first
      final groupOrderId = _groupOrderId ??
          widget.orderData?['groupOrderId']?.toString();
          ...
          child: TabBarView(
            ...
            children: [
              ...
              _NewCardTab(
                ...
                orderTotal:    widget.orderTotal,
                onSaveToggle: (v) {
                  debugPrint('💳 [SaveToggle] Save card toggled → $v');
